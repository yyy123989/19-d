...
static void APP_TIM4_Init(void)
{
  uint32_t prescaler = (APP_TIM_CLOCK_HZ / APP_DRG_TIMER_TICK_HZ);
  uint32_t period = ((APP_DRG_TIMER_TICK_HZ * APP_DRG_DIRECTION_MS) / 1000U);
...
int main(void)
{
  ...
  uint32_t last_draw_tick = HAL_GetTick();
  uint32_t last_serial_tick = HAL_GetTick();
  int16_t snapshot_samples[AD7606B_CHANNEL_COUNT];
  uint32_t fail_count;
  uint32_t now;
  uint8_t last_ok;
  uint8_t draw_due;
  ...
  while (1)
  {
    ...
    if (draw_due != 0U) {
      DDS_AppDraw(snapshot_samples, last_ok, ad_sample_rate_hz, fail_count);
      last_draw_tick = now;
    }

    if (serial_due != 0U) {
      SerialWave_Tick(now, snapshot_samples);
      SerialScreen_Tick(now, snapshot_samples, last_ok, ad_sample_rate_hz, fail_count);
